...
import '../../layouts/items/textboxes/BaseEditText.dart';
import '../../layouts/items/textboxes/CustomTextbox.dart';
...
class _RefreshAppState extends State<RefreshApp> {
  ...
  late Currency currency=Currency(code: "AED", name: "UNITED ARAB DIRHAM", symbol: "AED", flag: "AED", number: 1, decimalDigits: 2, namePlural: "AED", symbolOnLeft: true, decimalSeparator: '.', thousandsSeparator: ",", spaceBetweenAmountAndSymbol: true);
  ...
  Widget bodyWidget=Container();
    ApiUser currentUser= ApiUser(id: -1, firstName: "", email: "",  createdAt: '', updatedAt:  '', phone: "",  fcmToken: "", levelId: 1);
...
///////// register Data
 ...
 // Widget activeServiceFragment=ForeignExchangeFragment();
  ...
  ShapeBorder border=RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(12)), side: BorderSide(width: 0,));
  ApiAppVariables apiAppVariables=ApiAppVariables();
  ApiHeaders apiHeaders=ApiHeaders(token: "",acceptCurrency: "USD",acceptLanguage: "en");
...
// This is what you're looking for!
  ...
  PromoCodeEditText promoCode=PromoCodeEditText(text: "Promo code", margin: 0, marginh: 0, focusNode:FocusNode(),isValidationDialog: true, controller: TextEditingController(),  maxLines: 1, width: 305, height: 35);
  resetList(int index){
    print("cleared!!!");
  }
  ...
  //  EasyLoading.show();
   ...
   //   EasyLoading.dismiss();
    ...
    //  RefreshApp.of(mcontext)!.apiAppVariables
      RefreshApp.of(mcontext)!.apiAppVariables=ApiAppVariables.fromJson(value.data['data']);
    ...
    //  log("apiAppVariables splash m-------------"+apiAppVariables.toJson().toString());
      ...
      RefreshApp.of(mcontext)!.closingSoonCampaignLayout=  Container(height: 250,child: MiniCampaignLayout(height: 210, Campaigns: RefreshApp.of(mcontext)!.apiAppVariables!.closingSoonCampaigns!));
      //  campagins!.clear();

      if(RefreshApp.of(mcontext)!.activeCampaignWidgets.isNotEmpty) {
        RefreshApp.of(mcontext)!.activeCampaignWidgets.clear();
      }
      RefreshApp.of(mcontext)!.apiAppVariables!.activeCampaigns!.forEach((element) {
      ...
      RefreshApp.of(mcontext)!.apiAppVariables!.levels!.forEach((element) {
        ...
        RefreshApp.of(mcontext)!.levelsWidgets.add(MembershipItem(
          ...
          level: element, height: 165,width: 1,
        ));
      });
      ...
      RefreshApp.of(mcontext)!.bannersLayout= BannerLayout(isRadius:false,height: 350,banners:  RefreshApp.of(mcontext)!.apiAppVariables.banners!,);
      RefreshApp.of(mcontext)!.soldoutLayout= Container(height: 300,child: SoldOutLayout(height: 250, Campaigns:   RefreshApp.of(mcontext)!.apiAppVariables.closedCampaigns!));
      RefreshApp.of(mcontext)!.winnersLayout= Container(height: 300,child: WinnersLayout(height: 250, Draws:   RefreshApp.of(mcontext)!.apiAppVariables.winners!));
      if(RefreshApp.of(mcontext)!.upcomigDrawsWidgets.isNotEmpty) {
        RefreshApp.of(mcontext)!.upcomigDrawsWidgets.clear();
      }
      RefreshApp.of(mcontext)!.apiAppVariables!.upcomingDraws!.forEach((element) {

        RefreshApp.of(mcontext)!.upcomigDrawsWidgets.add(DrawItem(

          drawItem: element, height: 300,width: 1,
        ));
      });
      ...
      RefreshApp.of(mcontext)!.apiAppVariables!.winners!.forEach((element) {
        ...
        RefreshApp.of(mcontext)!.winnersWidgets.add(WinnerItem(
          ...
          winnerItem: element, height: 340,width: 1,
        ));
      });
      ...
      RefreshApp.of(mcontext)!.apiAppVariables!.products!.forEach((element) {
        ...
        RefreshApp.of(mcontext)!.productsWidgets.add(ProductItem(
          ...
          product: element, height: 100,width: 1,
        ));
      ...
      RefreshApp.of(mcontext)!.apiAppVariables!.userCoupons!.forEach((element) {
        if(!campaignIdBlack.contains(element.campaign!.id)) {
          ...
          RefreshApp.of(mcontext)!.couponWidgets.add(CouponItem(
            ...
            coupon: element, height: 160, width: 1,
          ));
        }
      ...
      RefreshApp.of(mcontext)!.apiAppVariables!.notifications!.forEach((element) {
        ...
        RefreshApp.of(mcontext)!.notificationsWidgets.add(NotificationItem(
          ...
          notification: element, height: 90,width: 1,
        ));
      ...
      RefreshApp.of(mcontext)!.apiAppVariables!.wishList!.forEach((element) {
        ...
        RefreshApp.of(mcontext)!.wishListWidgets.add(CampaignItem(
          mycampagin: element.campaign, height: 580,width: 1,
        ));
  ...
  Widget build(BuildContext context) {
    ...
    // models.insert(0, 'ALL');
    // years.insert(0, 'ALL');
    return RefreshAppState(child: widget.child,currentUser:currentUser,bodyWidget:bodyWidget,);
  }
}
...
class RefreshAppState extends InheritedWidget {
  ...
  static RefreshAppState? of(BuildContext context) {
    final result = context.dependOnInheritedWidgetOfExactType<RefreshAppState>();
  ...
  bool updateShouldNotify(RefreshAppState old) {
    return (currentUser != old.currentUser||currentUser != old.currentUser||bodyWidget!=old.bodyWidget);
  }
}
