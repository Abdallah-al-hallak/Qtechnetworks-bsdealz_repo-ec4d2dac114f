...
class MembershipItem extends StatefulWidget implements PreferredSizeWidget {
  ...
  MembershipItem({Key? key, required this.height, required this.width, required this.level,}) : super(key: key);
  @override
  ...
  Widget build(BuildContext context) {
    ...
    return
      GestureDetector(
          ...
          child:MembershipItemSingle(height: widget.height,width: widget.width,text: "555", level: widget.level,)
...
class MembershipItemSingle extends StatefulWidget {
  ...
  final String text;
  final ApiLevel level;
  const MembershipItemSingle({Key? key,required  this.height,required this.width,required this.text, required this.level}) : super(key: key);
  @override
...
class MembershipItemSingleState extends State<MembershipItemSingle> {
  ...
  Widget build(BuildContext context) {
    ...
    String levelhint="${AppSettingTheme.getTheme(context,Config.EARN_KEY,Config.EARN_VALUE,)} ${AppSettingTheme.getTheme(context,Config.FROM_KEY,Config.FROM_VALUE,)} ${widget.level.startThreshold!.replaceAll(RegExp(r'([.]*0+)(?!.*\d)'), '')}-${widget.level.endThreshold!.replaceAll(RegExp(r'([.]*0+)(?!.*\d)'), '')} to reach this level";
                                ...
                                Column(
                                  children: [
                                    Container(
                                      ...
                                      child: Card(
                                        elevation: 0,
                                        color:  Color(int.parse(widget.level.centerBoxColor.toString())),
                                        shape: shapeBorder1,
                                        child: Column(
                                          ...
                                          children: [
                                            BaseText(
                                              color:Color(int.parse(widget.level.contentFontColor.toString())),
                                              text:"${widget.level.name}",
                                              margin: 10,
                                              marginh: 25,
                                              fontSize: 21,
