    ...
    return
      ...
      child:Center(
        ...
        children: [
          ...
          LoginForm1(destination: widget.destination,)
            ,),
        ],
          ),
        ),
...
class LoginForm1State extends State<LoginForm1> {
 ...
 // TextboxPhone phone=TextboxPhone(text:"phone",margin: 5,marginh: 20,controller: TextEditingController(), validator: '', countryCode: '+962',);
  BaseEditText emailEditText=BaseEditText(text: "Email", margin: 20, marginh: 20, focusNode:FocusNode(),isValidationDialog: true,validationType: ValidationType.EMAIL, controller: TextEditingController(), type: TextBoxType.EMAIL, maxLines: 1, width: 305, height: 55);
  //Text(text:"Email",margin: 5,marginh: 20,controller: TextEditingController(), validator: '', countryCode: '+962', type: TextBoxType.EMAIL,);
  TextboxPassword pass=TextboxPassword(text:"pasword",margin: 20,marginh: 20,controller: TextEditingController(), validator: '',focusNode:FocusNode());

  @override
  void initState() {
    // TODO: implement initState
    super.initState();
     emailEditText=BaseEditText(text: "${AppSettingTheme.getTheme(context,Config.EMAIL_KEY,Config.EMAIL_VALUE,)}", margin: 20, marginh: 20, focusNode:FocusNode(),isValidationDialog: true,validationType: ValidationType.EMAIL, controller: TextEditingController(), type: TextBoxType.EMAIL, maxLines: 1, width: 305, height: 55);
    //Text(text:"Email",margin: 5,marginh: 20,controller: TextEditingController(), validator: '', countryCode: '+962', type: TextBoxType.EMAIL,);
     pass=TextboxPassword(text:"${AppSettingTheme.getTheme(context,Config.PASSWORD_KEY,Config.PASSWORD_VALUE,)}",margin: 10,marginh: 20,controller: TextEditingController(), validator: '',focusNode:FocusNode());
  ...
  Widget build(BuildContext context) {
    ...
    pass.text="${AppSettingTheme.getTheme(context,Config.PASSWORD_KEY,Config.PASSWORD_VALUE,)}";
          ...
          child: Container(
            ...
            child: ListView(
                ...
                children: <Widget>[
                  emailEditText,
                  pass,
                  Container(padding:EdgeInsets.only(right: 10,bottom: 20),margin:EdgeInsets.all(2),alignment:Alignment.centerRight,child:FooterText(text: "${AppSettingTheme.getTheme(context,Config.FORGOT_PASSWORD_KEY,Config.FORGOT_PASSWORD_VALUE,)}",onPressed: (){ Navigator.push(context, MaterialPageRoute(builder: (context)=>ForgotPassword()));}, clickable: true,
                      )),
                  MainButton(
                    text: "${AppSettingTheme.getTheme(context,Config.LOGIN_KEY,Config.LOGIN_KEY,)}",onPressed: () async {
                    setState(() {
           ...
           //   Navigator.push(context, MaterialPageRoute(builder: (context)=>Signup()));
                  MainButton(
                    ...
                    text: "CONTINUE AS GUEST",onPressed: (){
                    ...
                    child: Row(
                      ...
                      children: [
                        Container(
                            ...
                            alignment:Alignment.center,child:Text("${AppSettingTheme.getTheme(context,Config.DONT_HAVE_ACCOUNT_KEY,Config.DONT_HAVE_ACCOUNT_VALUE,)}",
                        ...
                        InkWell(
                          ...
                          child: Container(alignment:Alignment.center,child:Text("${AppSettingTheme.getTheme(context,Config.CREATE_ONE_KEY,Config.CREATE_ONE_VALUE,)}",
  ...
  void loginRequest(){
    ...
    var response =  dio.post("/login",
        data: { "email":emailEditText.controller.text.toString(),
          "password": pass.controller.text.toString(),}).then((value) async {
      setState(()  {
        ...
        RefreshApp.of(context)!.apiHeaders.token=token;
          ...
          EasyLoading.dismiss();
          MainDialog.showMyDialog(MainDialog(title: "${AppSettingTheme.getTheme(context,Config.FAILED_KEY,Config.FAILED_VALUE,)}",
            text: "${value.data['message'].toString()}",
            descriptions: "${value.data['message'].toString()}",type: DialogType.INFO,customWidget: Container(),),context);
      ...
      MainDialog.showMyDialog(MainDialog(title: "Failed",text: "Dismiss",
        descriptions: "Request Error!",type: DialogType.ERROR,customWidget: Container(),),context);
