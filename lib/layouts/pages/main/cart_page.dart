...
class _CartPageState extends State<CartPage> {
  ...
  TitleText nametxt = TitleText(text: "Username", onPressed: (){}, clickable: false);
...
applyPromoCode(){
    ...
    return dio.get('showcart?promo_code='+RefreshApp.of(context)!.promoCode.controller.text.toString()+'&use_points=${usePoints}').then((value) {
      setState(() {
 ...
 checkPromoCode(BuildContext context){
    HttpAPI http=HttpAPI();
    Dio dio=http.Inisalize(context);
    return dio.post('checkpromocode',data: {"promo_code":RefreshApp.of(context)!.promoCode.controller.text.toString()}).then((value) {
      setState(() {
        if(value.data['status']=="success"){
          promoStatus=Text("valid",style: TextStyle(color: Colors.green),);
        }else{
          promoStatus=Text("Invalid",style: TextStyle(color: Colors.red),);
        }
      });
    });
  }
submitItems(){
    ...
    RefreshApp.of(context)!.apiAppVariables.cart!.items!.forEach((element) {
      ApiCartItemR c =new ApiCartItemR(campaignId: element.campaignId,quantity:element.quantity);
      m.add(c);
    });
    List<Map<String, dynamic>> itemsList = m.map((model) => model.toJson()).toList();
  ...
  Widget build(BuildContext context) {
   ...
   // ApiCart cart=RefreshApp.of(context)!.apiAppVariables!.cart ?? ApiCart(items: []);
    print("---------------------------length="+RefreshApp.of(context)!.apiAppVariables!.cart!.items!.length.toString());
if(RefreshApp.of(context)!.apiAppVariables!.cart!.items!.length<1){
  cartWidget=ListView(children: [
    ...
    AppIcon(icon: AppIcon.CART_PATH, size: 70,color: Colors.grey,),
    Center(
      child: BaseText(
        color: Colors.grey,
    ...
    MainButton(onPressed: (){   Navigator.pop(context);
    setState(() {
 ...
 return Scaffold(
  ...
  cartItems.clear();
RefreshApp.of(context)!.apiAppVariables!.cart!.items!.forEach((element) {
  cartItems.add(CartItem(height: 140,width: 1,cartItem:  element),);
});
  cartWidget=ListView(
    ...
    if(usePoints) {
      myWalletTile =                           Container(
        ...
        child:     ListTile(
            tileColor: Colors.grey[100],
            title:  Container(),
            trailing:  Container(  height: 20,child: FooterText(text: "${RefreshApp.of(context)!.apiAppVariables.cart!.pointsDiscount}",clickable: false,onPressed: (){})),
            leading:Container(  height: 20,child: FooterText(text: "My wallet",clickable: false,onPressed: (){})),
            onTap: () {
      ...
      String p="...";
      if(RefreshApp.of(context)!.apiAppVariables.cart!.pointsToSubtract!=null){
        p=RefreshApp.of(context)!.apiAppVariables.cart!.pointsToSubtract!.toString();
      }
      pointsToSubtract =      Container(
        height: 20,
        margin: EdgeInsets.only(left:30,right: 30,top: 20),
        alignment: Alignment.topLeft,
        child:     Text("*You will use ${p} points",style: TextStyle(color: Colors.red,fontSize: 9),),
      );
    }
    return Scaffold(
                            ...
                            child: Card(
                              ...
                              child:  Container(
                                ...
                                child:     ListTile(
                                    ...
                                    title:  SettingText(text: "Donate the product",onPressed: (){},clickable: false),
                            ...
                            child: Card(
                              ...
                              child:  Container(
                                ...
                                child:     ListTile(
                                    ...
                                    title:  SettingText(text: "Use my points",onPressed: (){},clickable: false),
                                    leading:Transform.scale(
                                        scale: 1.4,
                                        child: Switch(
                          ...
                          child:
                              Row(children: [
                                Container(
                                    ...
                                    child: AppIcon(icon:AppIcon.PROMO_CODE_PATH,size: 25)),
                               Expanded(child: Container(child: RefreshApp.of(context)!.promoCode)),
                                Container(
                            ...
                            children: <Widget>[
                              ...
                              Container(
                                ...
                                child:     ListTile(
                                    tileColor: Colors.grey[100],
                                    title:  Container(),
                                    trailing:  Container(  height: 20,child: FooterText(text: "${RefreshApp.of(context)!.apiAppVariables.cart!.total}",clickable: false,onPressed: (){})),//${RefreshApp.of(context)!.apiAppVariables.cart!.subtotal}
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child:     ListTile(
                                    tileColor: Colors.grey[100],
                                    title:  Container(),
                                    trailing:  Container(  height: 20,child: FooterText(text: "${RefreshApp.of(context)!.apiAppVariables.cart!.total}",clickable: false,onPressed: (){})),// ${RefreshApp.of(context)!.apiHeaders.acceptCurrency}${RefreshApp.of(context)!.CartTotal} ${RefreshApp.of(context)!.apiAppVariables.cart!.total}
                            ...
                            onPressed: (){
                              if(kIsWeb){
                                WebDialog.showMyDialog(WebDialog(title: "",text: "Dismiss",
                                  descriptions: "",customWidget: CheckoutPage(),),context);
                              }else {
                                submitItems();
